...
<table className='marker_table'>
	...
	<% it.type.allMarkers.forEach((m) => { %>
		<tr className='mark_inherited hidden'>
			...
			<td className="p-2 align-top"><span className="nx-bg-primary-100 rounded-md px-3 py-1 nx-text-primary-800 font-semibold text-center text-sm whitespace-nowrap mr-2 inline-block">&larr; sys::Marker</span><%~ it.hsDocs.get(m)?.replaceAll('\n', ' ') %></td>
		</tr>
	<% }) %>
	<%} %>
	</tbody>
