...
<div className={`${classes.diagramContainer} nx-bg-primary-700/5 dark:nx-bg-primary-300/10`}>
	<div className='text-xs px-4 py-2 font-semibold nx-bg-primary-700/5 nx-text-gray-700 dark:nx-bg-primary-300/10 dark:nx-text-gray-200 nx-rounded-t-xl'>
	Entity Hierachy
	</div>
...
<table className='marker_table'>
	...
	<% it.type.selfMarkers.forEach((m) => { %>
		<tr>
			<td className="p-2 align-top">
				<span className="nx-bg-primary-100 dark:nx-bg-primary-400/10 rounded-md px-3 py-1 nx-text-primary-800 dark:nx-text-primary-600 w-full inline-block font-semibold text-center text-sm"><%= m %></span>
			</td>
	...
	<% it.type.inheritedMarkers.forEach(({name, type}) => { %>
		<tr className='mark_inherited'>
			<td className="p-2 align-top">
				<span className="border nx-border-primary-100 dark:nx-border-primary-800 rounded-md px-3 py-1 w-full inline-block font-semibold text-center text-sm"><%= name %></span>
			</td>
			<td className="p-2 align-top"><span className="nx-bg-primary-100 dark:nx-border-primary-400/10 rounded-md px-3 py-1 nx-text-primary-800 dark:nx-text-primary-600 font-semibold text-center text-sm whitespace-nowrap mr-2 inline-block">&larr; <%= type %></span><%= it.utils.docWithFixedLinks(it.hsDocs.get(name)) %></td>
		</tr>
	<% }) %>
	<%} %>
	</tbody>
...
<div className={`${classes.summaryBox} flex flex-col bg-emerald-700/5 dark:nx-bg-primary-300/10 nx-rounded-xl mt-10`}>
	<div className='flex text-sm px-4 py-2 font-semibold bg-emerald-700/5 rounded-t-xl text-emerald-800/70'>
	## Point Summary
